...
public class LinhKien_Dao {
	...
	public int soLuongLK() {
		...
		try {
			PreparedStatement statement = con
					.prepareStatement("select count(maLinhKien) as Dem from LinhKien");
	...
	public List<LinhKien> getLinhKien() {
		...
		Connection con  = new ConnectDB().getConnect();
		try {
			PreparedStatement statement = con.prepareStatement("select * from LinhKien join NhaCungCap on LinhKien.maNCC=NhaCungCap.maNCC");
			ResultSet rs = statement.executeQuery();
			while(rs.next()) {
				list.add(new LinhKien(new NhaCungCap(rs.getString("tenNCC")), rs.getString("maLinhKien"), rs.getString("tenLinhKien"), rs.getString("loaiLinhKien"), rs.getInt("soLuongTon"), rs.getDouble("giaBan"), rs.getInt("baoHanh"), rs.getString("moTa"), rs.getDate("ngayNhap")));
	...
	public List<LinhKien> phanTrangLinhKien(int fn, int ln) {
		...
		String sql = "select *from(select ROW_NUMBER() over (order by MaLinhKien) as STT, ncc.maNCC, lk.maLinhKien,"
				...
				+ "join NhaCungCap ncc on lk.maNCC=ncc.maNCC) as PhanTrang where PhanTrang.STT Between " + fn + " and " +ln;
		try {
	...
	public boolean updateLinhKien(LinhKien lk) {
		Connection con = new ConnectDB().getConnect();
		try {
			PreparedStatement statement = con.prepareStatement("update LinhKien set maNCC=?,tenLinhKien=?,loaiLinhKien=?,soLuongTon=?,giaBan=?,baoHanh=?,moTa=?,ngayNhap=? where maLinhKien=?");
			statement.setString(1, lk.getNhaCC().getMaNCC());
	...
	public List<LinhKien> timLinhKienTheoTen(String ten,String loai,String thuongHieu,double s1,double s2) {
		...
		try {
			...
			String sql = "select * from LinhKien join NhaCungCap on LinhKien.maNCC=NhaCungCap.maNCC where tenLinhKien like N'%" +ten + "%'" + loaiLK + thuongHieuLK +giaBanLK;	
	...
	public List<LinhKien> timLinhKien(String ten,String loai,String thuongHieu) {
		...
		try {
			String thuongHieuLK = "and tenNCC like N'%" +thuongHieu + "%'";
			String loaiLK = "and loaiLinhKien like N'%" +loai + "%'";
			String sql = "select * from LinhKien join NhaCungCap on LinhKien.maNCC=NhaCungCap.maNCC where tenLinhKien like N'%" +ten + "%'" + loaiLK + thuongHieuLK;	
